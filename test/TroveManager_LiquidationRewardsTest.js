...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  let contracts
  let collateralAmount = dec(40000, 30);
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    await borrowerOperations.addColl(B, addedColl1, B, B, { from: B })
    ...
    await borrowerOperations.addColl(B, addedColl2, B, B, { from: B })
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    // // Bob adds 1 ETH to his trove
    await borrowerOperations.addColl(B, dec(1, 'ether'), B, B, { from: B  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    await borrowerOperations.addColl(bob, addedColl, bob, bob, { from: bob })

    // Alice withdraws LUSD
    await borrowerOperations.withdrawLUSD(alice, th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, { from: alice })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const { collateral: C_coll } = await openTrove({ collatAmount: _998_Ether, extraLUSDAmount: dec(110, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ collatAmount: dec(1000, 'ether'), ICR: toBN(dec(200, 16)), extraLUSDAmount: dec(110, 18), extraParams: { from: dennis } })
    ...
    await borrowerOperations.addColl(carol, dec(1, 'ether'), carol, carol, { from: carol  })
    ...
    const { collateral: E_coll } = await openTrove({ collatAmount: entireSystemColl_2, ICR: toBN(dec(200, 16)), extraParams: { from: erin } })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(alice, addedColl, alice, alice, { from: alice })
    await borrowerOperations.addColl(bob, addedColl, bob, bob, { from: bob })
    await borrowerOperations.addColl(carol, addedColl, carol, carol, { from: carol })
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    await borrowerOperations.withdrawColl(bob, withdrawnColl, bob, bob, { from: bob })
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    await borrowerOperations.withdrawColl(carol, C_withdrawnColl, carol, carol, { from: carol })
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const withdrawnColl = toBN(dec(500, 'finney'))
    await borrowerOperations.withdrawColl(alice, withdrawnColl, alice, alice, { from: alice })
    await borrowerOperations.withdrawColl(bob, withdrawnColl, bob, bob, { from: bob })
    await borrowerOperations.withdrawColl(carol, withdrawnColl, carol, carol, { from: carol })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    await borrowerOperations.addColl(bob, B_addedColl, bob, bob, { from: bob })
    ...
    await borrowerOperations.addColl(dennis, D_addedColl, dennis, dennis, { from: dennis })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    C: 23.902 ETH
    */
    const { collateral: A_coll } = await openTrove({ collatAmount: toBN('450000000000000000000'), ICR: toBN(dec(90000, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ collatAmount: toBN('8901000000000000000000'), ICR: toBN(dec(1800000, 16)), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ collatAmount: toBN('23902000000000000000'), ICR: toBN(dec(4600, 16)), extraParams: { from: carol } })
    ...
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ collatAmount: toBN(dec(35, 15)), extraLUSDAmount: dec(100, 18), extraParams: { from: dennis } })
    ...
    F: 0.0007 ETH
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ collatAmount: toBN(dec(1, 22)), extraLUSDAmount: dec(100, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ collatAmount: toBN('700000000000000'), extraLUSDAmount: dec(100, 18), extraParams: { from: freddy } })
