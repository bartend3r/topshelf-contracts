...
contract('StabilityPool - LQTY Rewards', async accounts => {
  ...
  let issuance_M5
  let issuance_M6
  let collateralAmount = dec(2, 40);
  let approvalAmount = dec(2, 40);
  ...
  describe("LQTY Rewards", async () => {
    ...
    beforeEach(async () => {
      ...
      let foo = [
        owner,
        whale,
        A, B, C, D, E, defaulter_1, F, G, H,
         defaulter_2, defaulter_3, defaulter_4, defaulter_5, defaulter_6,
        frontEnd_1, frontEnd_2, frontEnd_3
      ];
      for (account of foo) {
    ...
    it("withdrawFromSP(): reward term G does not update when no LQTY is issued", async () => {
      await borrowerOperations.openTrove(A, th._100pct, dec(1000, 'ether'), dec(10000, 18), A, A, { from: A })
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: A })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      let initialIssuance = await communityIssuanceTester.totalLQTYIssued()
      assert.equal(initialIssuance, 0)

      // Whale opens Trove with 10k ETH
      await borrowerOperations.openTrove(whale, th._100pct, dec(10000, 'ether'), dec(10000, 18), whale, whale, { from: whale })

      await borrowerOperations.openTrove(A, th._100pct, dec(100, 'ether'), dec(1, 22), A, A, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(100, 'ether'), dec(1, 22), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(100, 'ether'), dec(1, 22), C, C, { from: C })
      await borrowerOperations.openTrove(D, th._100pct, dec(100, 'ether'), dec(1, 22), D, D, { from: D })

      // Check all LQTY balances are initially 0
      assert.equal(await lqtyToken.balanceOf(whale), 0)
      ...
      // A, B, C deposit
      // WHY DOES A GET AN ISSUANCE HERE BUT THE NORMAL TEST DOESNT?
      ...
      const B_LQTYGain_0yr = await stabilityPool.getDepositorLQTYGain(B)
      const C_LQTYGain_0yr = await stabilityPool.getDepositorLQTYGain(C)
      // console.log('A_LQTYGain_0yr', A_LQTYGain_0yr.toString())
      // console.log('B_LQTYGain_0yr', B_LQTYGain_0yr.toString())
      // console.log('C_LQTYGain_0yr', C_LQTYGain_0yr.toString())
      // One year passes
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(A, th._100pct, dec(200, 'ether'), dec(10000, 18), A, A, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(300, 'ether'), dec(20000, 18), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(400, 'ether'), dec(30000, 18), C, C, { from: C })
      await borrowerOperations.openTrove(D, th._100pct, dec(100, 'ether'), dec(10000, 18), D, D, { from: D })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(B, th._100pct, dec(300, 'ether'), dec(20000, 18), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(400, 'ether'), dec(30000, 18), C, C, { from: C })
      await borrowerOperations.openTrove(D, th._100pct, dec(500, 'ether'), dec(40000, 18), D, D, { from: D })
      await borrowerOperations.openTrove(E, th._100pct, dec(600, 'ether'), dec(40000, 18), E, E, { from: E })

      await borrowerOperations.openTrove(defaulter_1, th._100pct, dec(300, 'ether'), await getOpenTroveLUSDAmount(dec(30000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
    ...
    it('withdrawFromSP(): Depositor withdraws correct LQTY gain after serial pool-emptying liquidations. No front-ends.', async () => {
      ...
      for (account of depositors_1) {
        await borrowerOperations.openTrove(account, th._100pct, dec(200, 'ether'), dec(10000, 18), account, account, { from: account })
    ...
    it('LQTY issuance for a given period is not obtainable if the SP was empty during the period', async () => {
      ...
      await borrowerOperations.openTrove(A, th._100pct, dec(200, 'ether'), dec(16000, 18), A, A, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(100, 'ether'), dec(10000, 18), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(200, 'ether'), dec(16000, 18), C, C, { from: C })
    ...
    it("withdrawFromSP(): Several deposits of 100 LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      await borrowerOperations.openTrove(whale, th._100pct, dec(100, 'ether'), await getOpenTroveLUSDAmount(dec(10000, 18)), whale, whale, { from: whale })

      const fiveDefaulters = [defaulter_1, defaulter_2, defaulter_3, defaulter_4, defaulter_5]

      await borrowerOperations.openTrove(A, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: A  })
      await borrowerOperations.openTrove(B, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: B  })
      await borrowerOperations.openTrove(C, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: C  })
      await borrowerOperations.openTrove(D, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: D  })
      await borrowerOperations.openTrove(E, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: E  })
      await borrowerOperations.openTrove(F, th._100pct, dec(10000, 'ether'), dec(10000, 18), ZERO_ADDRESS, ZERO_ADDRESS, { from: F  })

      for (const defaulter of fiveDefaulters) {
        // Defaulters 1-5 each withdraw to 9999.9 debt (including gas comp)
        await borrowerOperations.openTrove(defaulter, th._100pct, dec(100, 'ether'), await getOpenTroveLUSDAmount('9999900000000000000000'), defaulter, defaulter, { from: defaulter })
      }
      ...
      // await contracts.collateral.approve(borrowerOperations.address, collateralAmount, { from: defaulter_6 } )

      // Defaulter 6 withdraws to 10k debt (inc. gas comp)
      await borrowerOperations.openTrove(defaulter_6, th._100pct, dec(100, 'ether'), await getOpenTroveLUSDAmount(dec(10000, 18)), defaulter_6, defaulter_6, { from: defaulter_6 })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      await borrowerOperations.openTrove(A, th._100pct, dec(100, 'ether'), dec(10000, 18), A, A, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(100, 'ether'), dec(10000, 18), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(100, 'ether'), dec(10000, 18), C, C, { from: C })
      await borrowerOperations.openTrove(D, th._100pct, dec(100, 'ether'), dec(10000, 18), D, D, { from: D })
      await borrowerOperations.openTrove(E, th._100pct, dec(100, 'ether'), dec(10000, 18), E, E, { from: E })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. Front ends and kickback rates", async () => {
      ...
      await borrowerOperations.openTrove(A, th._100pct, dec(200, 'ether'), dec(10000, 18), A, A, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(800, 'ether'), dec(60000, 18), B, B, { from: B })
      await borrowerOperations.openTrove(C, th._100pct, dec(400, 'ether'), dec(30000, 18), C, C, { from: C })
      await borrowerOperations.openTrove(D, th._100pct, dec(500, 'ether'), dec(40000, 18), D, D, { from: D })

      await borrowerOperations.openTrove(E, th._100pct, dec(400, 'ether'), dec(30000, 18), E, E, { from: E })

      // D1, D2, D3 open troves with total debt 50k, 30k, 10k respectively (inc. gas comp)
      await borrowerOperations.openTrove(defaulter_1, th._100pct, dec(500, 'ether'), await getOpenTroveLUSDAmount(dec(50000, 18)), defaulter_1, defaulter_1, { from: defaulter_1 })
      await borrowerOperations.openTrove(defaulter_2, th._100pct, dec(200, 'ether'), await getOpenTroveLUSDAmount(dec(20000, 18)), defaulter_2, defaulter_2, { from: defaulter_2 })
      ...
      const startTime = await communityIssuanceTester.issuanceStartTime()
      const currentTime = await th.getLatestBlockTimestamp(web3)
    ...
    it("withdrawFromSP(): Several deposits of 10k LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      for (const defaulter of _4_Defaulters) {
        ...
        await borrowerOperations.openTrove(defaulter, th._100pct, dec(100, 'ether'), await getOpenTroveLUSDAmount(dec(99999, 17)), defaulter, defaulter, { from: defaulter })
      }
      ...
      await borrowerOperations.openTrove(A, th._100pct, dec(200, 'ether'), dec(5000, 18), A, A, { from: A })
      await stabilityPool.provideToSP(dec(5000, 18), frontEnd_1, { from: A })
      await borrowerOperations.openTrove(B, th._100pct, dec(200, 'ether'), dec(5000, 18), B, B, { from: B })
      ...
      // C provides to SP
      await borrowerOperations.openTrove(C, th._100pct, dec(200, 'ether'), dec(99999, 17), C, C, { from: C })
      ...
      // D provides to SP
      await borrowerOperations.openTrove(D, th._100pct, dec(200, 'ether'), dec(99999, 17), D, D, { from: D })
      ...
      // E provides to SP
      await borrowerOperations.openTrove(E, th._100pct, dec(200, 'ether'), dec(99999, 17), E, E, { from: E })
